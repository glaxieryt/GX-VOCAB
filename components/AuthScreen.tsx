...
const AuthScreen: React.FC<AuthScreenProps> = ({ onAuthSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let user;
            if (isSignUp) {
                user = await signUp(username, password, isPublic);
            } else {
                    ...
                    {isSignUp && (
                        <div className="animate-fadeIn space-y-4">
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group">
                                <div className={`w-5 h-5 border flex items-center justify-center transition-colors ${isPublic ? 'bg-black border-black dark:bg-white dark:border-white' : 'bg-white border-zinc-300 dark:bg-zinc-900 dark:border-zinc-600'}`}>
                                    {isPublic && (
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5 text-white dark:text-black">
                                ...
                                />
                                <span className="text-xs text-zinc-600 dark:text-zinc-400 select-none group-hover:text-black dark:group-hover:text-white transition-colors">
